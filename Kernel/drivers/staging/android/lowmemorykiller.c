...
static int lowmem_oom_adj_to_oom_adj(int oom_adj)
{
	if (oom_adj == OOM_ADJUST_MAX)
		return OOM_ADJUST_MAX;
	else
		return (oom_adj * OOM_ADJUST_MAX) / -OOM_DISABLE;
}
...
static void lowmem_autodetect_oom_adj_values(void)
{
	...
	oom_adj = lowmem_oom_adj_to_oom_adj(oom_adj);
	if (oom_adj <= OOM_ADJUST_MAX)
		return;

	lowmem_print(1, "lowmem_shrink: convert oom_adj to oom_adj:\n");
	for (i = 0; i < array_size; i++) {
		oom_adj = lowmem_adj[i];
		oom_adj = lowmem_oom_adj_to_oom_adj(oom_adj);
		lowmem_adj[i] = oom_adj;
		lowmem_print(1, "oom_adj %d => oom_adj %d\n",
			     oom_adj, oom_adj);
	}
}
