				    ...
				    struct wl1271_static_data *static_data)
{
	...
	pr_info("%s wlcore_identify_fw = %d\n", __func__, ret);
	if (ret < 0)
		goto out;
out:
	return ret;
...
static int wlcore_boot_static_data(struct wl1271 *wl)
{
	...
	pr_info("%s wlcore_read = %d\n", __func__, ret);
	if (ret < 0)
		goto out_free;

	ret = wlcore_boot_parse_fw_ver(wl, static_data);
	pr_info("%s wlcore_boot_parse_fw_ver = %d\n", __func__, ret);
	if (ret < 0)
		goto out_free;

	ret = wlcore_validate_fw_ver(wl);
	pr_info("%s wlcore_validate_fw_ver = %d\n", __func__, ret);
	if (ret < 0)
		goto out_free;

	ret = wlcore_handle_static_data(wl, static_data);
	pr_info("%s wlcore_handle_static_data = %d\n", __func__, ret);
	if (ret < 0)
		goto out_free;
					     ...
					     size_t fw_data_len, u32 dest)
{
	...
	pr_info("%s wlcore_set_partition = %d\n", __func__, ret);
	if (ret < 0)
		goto out;
...
int wlcore_boot_run_firmware(struct wl1271 *wl)
{
	...
	pr_info("%s wl1271_boot_set_ecpu_ctrl = %d\n", __func__, ret);
	if (ret < 0)
		return ret;

	ret = wlcore_read_reg(wl, REG_CHIP_ID_B, &chip_id);
	pr_info("%s wlcore_read_reg(REG_CHIP_ID_B) = %d\n", __func__, ret);
	if (ret < 0)
		return ret;
	...
	pr_info("%s wlcore_read_reg(REG_COMMAND_MAILBOX_PTR) = %d\n", __func__, ret);
	if (ret < 0)
		return ret;
	...
	pr_info("%s wlcore_read_reg(REG_EVENT_MAILBOX_PTR) = %d\n", __func__, ret);
	if (ret < 0)
		return ret;
	...
	pr_info("%s wlcore_boot_static_data = %d\n", __func__, ret);
	if (ret < 0) {
