...
static void wlcore_nvs_cb(const struct firmware *fw, void *context)
{
	...
	pr_info("%s platform_get_resource = %d\n", __func__, ret);
	if (!res) {
	...
	ret = wl12xx_set_power_on(wl);
	pr_info("%s wl12xx_set_power_on = %d\n", __func__, ret);
	if (ret < 0)
		goto out_free_nvs;

	ret = wl12xx_get_hw_info(wl);
	pr_info("%s wl12xx_get_hw_info = %d\n", __func__, ret);
	if (ret < 0) {
	...
	pr_info("%s request_threaded_irq = %d\n", __func__, ret);
	if (ret < 0) {
	...
	pr_info("%s identify_chip = %d\n", __func__, ret);
	if (ret < 0)
		goto out_irq;

	ret = wl1271_init_ieee80211(wl);
	pr_info("%s wl1271_init_ieee80211 = %d\n", __func__, ret);
	if (ret)
		goto out_irq;

	ret = wl1271_register_hw(wl);
	pr_info("%s wl1271_register_hw = %d\n", __func__, ret);
	if (ret)
		goto out_irq;

	ret = wlcore_sysfs_init(wl);
	pr_info("%s wlcore_sysfs_init = %d\n", __func__, ret);
	if (ret)
		goto out_unreg;
